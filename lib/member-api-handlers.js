 ...
 * query: kind=expert|certificate, documentId=专家或证书的 documentId
 */
async function download(req) {
  ...
  try {
    ...
    if (kind === 'expert') {
      ...
      if (!f?.url) return { status: 404, body: { success: false, message: '未找到证书文件' } };
      fileUrl = f.url.startsWith('http') ? f.url : `${strapiOrigin()}${f.url}`;
      filename = (f.name || row.certificateNumber || 'certificate') + (f.ext || '');
    } else {
